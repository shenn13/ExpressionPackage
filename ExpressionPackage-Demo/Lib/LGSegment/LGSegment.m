...
#define LG_ButtonColor_Selected [UIColor colorWithRed:0.0/255 green:197.0/255 blue:11.0/255 alpha:1]
#define LG_ButtonColor_UnSelected [UIColor colorWithRed:153.0/255 green:153.0/255 blue:153.0/255 alpha:100]
#define LG_BannerColor [UIColor colorWithRed:0.0/255 green:197.0/255 blue:11.0/255 alpha:1]
...
-(void)commonInit {
    NSMutableArray *titleList = [[NSMutableArray alloc]initWithObjects:@"热门配文",@"文字颜色",@"字体选择",@"背景颜色", nil];
...
- (void)createItem:(NSMutableArray *)item {
    ...
    CGFloat marginX = (self.frame.size.width - count * 90)/(count + 1);
    
    for (int i = 0; i<count; i++) {
        
        NSString *temp = [self.titleList objectAtIndex:i];
        //按钮的X坐标计算，i为列数
        CGFloat buttonX = marginX + i * (90 + marginX);
        UIButton *buttonItem = [[UIButton alloc] initWithFrame:CGRectMake(buttonX, 0, 90, self.frame.size.height)];
        buttonItem.tag = i + 1;
...
-(void)creatBanner:(CGFloat)firstX{
    ...
    LGLayer.frame = CGRectMake(firstX, self.frame.size.height - 6, 90, 5);
